...
scale = (zvals**-1)*(zvals**(-R))*2**-24

CIC = 4*scale*(COMB**M)*(INT**M)



#CIC = 2**-12*((1-zvals**-R)/(1-zvals**-1))**M*zvals**-12
#plt.plot(normf*125e6,10*np.log(abs(CIC)/(R**M)))
#plt.grid()

COMB1 = zvals**-R - zvals**(-2*R)
COMB = 1-zvals**-R
...
Lz = (2**-T)*((1-(zvals**-R))/(1-(zvals**-1)))**M #CIC Transfer Function Normalised
Lz_b = (2**-T)*((1-(z_b**-R))/(1-(z_b**-1)))**M #Bilinear transform

expDataCIC = np.transpose(genfromtxt("test_20221208_171446_Traces.csv", delimiter=','))
...
# CIC = ((1-zvals**-R)/(1-zvals**-1))**M
# #plt.plot(normf*125e6,10*np.log(abs(CIC)/(R**M)))
# #plt.grid()

# expDataCIC = np.transpose(genfromtxt("CM1R4064IC_Traces.csv", delimiter=','))


# figbz, axsbz = plt.subplots(2, sharex = True)
# axsbz[0].set_title("CIC Transfer Functions")
# #axsbz[0].plot(FREQ_NCO, 10*np.log10(np.abs(MAGS_CIC/MAGS_NCO)), label = "Vivado Sim. Transfer Function", c = "blue")
# axsbz[0].plot(normf*125e6,10*np.log(abs(CIC)/(R**M)), label = "Z-Domain Model", c = "orange")
# axsbz[0].plot(expDataCIC[0],expDataCIC[1]+1, label = "Experimental", c = "pink")
# #axsbz[1].plot(FREQ_NCO, np.degrees(PHASE_CIC-PHASE_NCO), c = "blue")
# axsbz[1].plot(normf*125e6, np.degrees(np.angle(CIC)), c = "orange")
# axsbz[1].plot(expDataCIC[0],expDataCIC[2], c = "pink")
# axsbz[0].set_xscale('log')
# axsbz[0].set_xlim(10, 1e4)
# axsbz[0].set_ylim([-50, 5])
# axsbz[0].legend()
# axsbz[0].grid()   
# axsbz[1].grid()  
